...
int main() {
        ...
        // ThreadPool
        {
            ...
            auto start = chrono::high_resolution_clock::now();
            {
                ...
                for (int i = 0; i < NUM_TASKS; i++) {
                    results.push_back(pool.executetasks(sortTask, i, SIZE, arr));
                }
            ...
            auto end = chrono::high_resolution_clock::now();
            tp_times.push_back(chrono::duration_cast<chrono::milliseconds>(end - start).count());
        }
    ...
    double avg_tp = accumulate(tp_times.begin(), tp_times.end(), 0.0) / NUM_RUNS;
